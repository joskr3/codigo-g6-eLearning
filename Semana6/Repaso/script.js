...
let contenedorInfo = document.getElementsByClassName(
...
function iniciarSesion() {
  ...
  if (usuarioValido && passwordValido) {
    ...
    document.querySelector("#username-usuario").innerHTML =
  ...
  arrayUsuarios.forEach((elementoArray) => {
    if (elementoArray.nombre === usuario.nombre && elementoArray.balance >= 0) {
      elementoArray.balance =
        elementoArray.balance -
        parseInt(document.getElementById("label-monto-retiro").value);
...
function depositar() {
  ...
  arrayUsuarios.forEach((elementoArray) => {
    if (elementoArray.nombre === usuario.nombre && elementoArray.balance >= 0) {
      elementoArray.balance =
        elementoArray.balance +
        parseInt(document.getElementById("operacion-deposito").value);
      // alert("Retiro realizado con exito");
      console.log(usuario.balance);
    }
  });
...
function irAFuncionDeConsulta() {
  ...
  if (usuario) {
    ...
    document.querySelector("#resultado-consulta").innerHTML =
      usuario.balance + "  " + usuario.moneda;
  }
}
